...
| `Editor/ExternalEditorController.swift` | Orchestrates Edit in Editor: capture the focused field, run the editor (directly for GUI editors, in a terminal for the rest), and write the result back on a clean exit. Its live edges are injectable (`Dependencies`), and the whole flow is tested against a fake field. |
...
| `Bookmarks/FaviconStore.swift`          | Fetches each bookmark's `/favicon.ico`, falling back to the icon its homepage declares (`FaviconLinkParser`); disk-caches the result, rows fall back to a generic glyph when there's none.                         |
| `Bookmarks/FaviconLinkParser.swift`     | Pulls the icon url out of a page's `<link rel="icon">` tag, for sites (Webflow, Squarespace, …) that don't serve a conventional `/favicon.ico`.                 |
